...
fn find_window<'a>(
    ...
    for name in names {
        if let Some(value) = rate_limit.get(*name) {
            ...
            if window.window_seconds.is_none_or(|seconds| {
                seconds == 0 || (expected_seconds > 0 && seconds.abs_diff(expected_seconds) <= 60)
            }) {
                return Some(value);
            }
        }
    ...
    for key in [
        ...
        for item in items {
            ...
            let matches_duration = window.window_seconds.is_some_and(|seconds| {
                expected_seconds > 0 && seconds.abs_diff(expected_seconds) <= 60
            });
