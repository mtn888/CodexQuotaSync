...
- 相邻两次可靠的成功响应中，若“执行中”减少或合并后的“待处理”增加，则发送系统通知；断联和失败请求不会通知。
...
1. 安装 APK。
2. 打开一次 `Codex Quota Sync`，在 Android 13 及以上系统中允许通知。
3. 长按手机桌面空白处，选择“小组件”。
4. 找到 `Codex Quota Sync`，选择 1×1、4×1 或 4×3 组件并拖到桌面。
5. 在配置页输入同步服务的 Base URL，例如 `http://nas.example.com:18080`。
6. 点击“保存并刷新小组件”。
...
应用只比较相邻两次成功且可信的活动状态：首次成功仅建立基线；同一数据版本不会重复提醒。满足下列任一条件时发送一条通知，两个条件同时发生时合并为一条：

- `executing` 比上次减少：提示有任务完成。
- `waitingOnApproval + waitingOnUserInput` 比上次增加：提示有新的待处理事项。

服务器无法访问、HTTP/JSON 失败、活动来源为 `unavailable`、活动状态标记为过期、切换服务器或版本没有前进时均不通知，也不会把断联误判成任务归零。通知点击后进入应用状态详情页。

Android 13 及以上必须授予通知权限。澎湃 OS 3 还需在系统设置中允许该应用的通知与悬浮通知；若希望后台轮询更稳定，请同时允许自启动并把电量策略设为“不限制”。系统或厂商仍可能延迟 WorkManager，因此通知时效取决于实际成功请求的时间，并非分钟级推送。
...
2. 允许 `Codex Quota Sync` 通知、悬浮通知、后台运行和自启动，并加入电池优化白名单。
